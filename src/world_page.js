...
WorldPage.prototype.fetch = function(world_name, callback) {
  var self = this;
  $.get('http://www.tibia.com/community/?subtopic=worlds&world=' + world_name).done(function(data, textStatus, jqXHR) {
    if (jqXHR.status === 200) {
    ...
    return callback('Fetch World wrong status ' + jqXHR.status, {});
  }).fail(function(data, textStatus, jqXHR) {
    return callback('Fetch World XHR failed ' + textStatus, {});
  });
};
...
WorldPage.prototype.query = function(world_name, callback) {
  ...
  if (!world || (world.time + self.cache_time < now && world.running === false)) {
    ...
    self.fetch(world_name, function(err, res) {
      world.running = false;
      if (err) {
        return callback(err, {});
      }
